...
fun CloudStorageView(
    ...
    val buckets by viewModel.cloudStorageEngine.buckets.collectAsState()

    LazyColumn(
        modifier = modifier
            .fillMaxSize()
            .padding(16.dp)
            .testTag("cloud_storage_view"),
        verticalArrangement = Arrangement.spacedBy(12.dp)
    ) {
        item {
            Text(
                text = "☁️ Multi-Cloud Storage (S3 / GCS / R2)",
        ...
        items(buckets) { bucket ->
            Surface(
                ...
                Column(modifier = Modifier.padding(14.dp)) {
                    Text("${bucket.provider.iconEmoji} ${bucket.name}", fontWeight = FontWeight.Bold, fontSize = 14.sp, color = Color(0xFF38BDF8))
                    Text("Region: ${bucket.region} • Objects: ${bucket.totalObjects}", fontSize = 12.sp, color = Color(0xFF94A3B8))
                }
            }
        }
    }
